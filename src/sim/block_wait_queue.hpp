...
public:
  ...
  /** @brief Return the compile-time number of jobs per block. @return Block
   * capacity as size_t. */
  static constexpr size_t block_size() { return BlockSize; }

  /** @brief Return log2(block_size()), used to map job IDs to blocks. @return
   * Bit shift as size_t. */
   ...
   */
  template <typename Func> void for_each_active(Func &&func) const;

  /** @brief Return jobs ever inserted, including jobs later removed. @return
   * Count as size_t. */
  size_t size() const { return m_total_jobs; }

  /** @brief Return jobs currently waiting in the queue. @return Active-job
   * count as size_t. */
  size_t active_count() const { return m_active_count; }

  /** @brief Report whether no active jobs remain. @return true when
   * active_count() is zero. */
  bool empty() const { return m_active_count == 0; }

  /** @brief Return accumulated backfill-search instrumentation. @return Stats
   * value snapshot. */
  struct Stats {
...
private:
  /** @brief Per-job data retained in one fixed-size queue block. */
  struct JobEntry {
  ...
  // Boost multi-index container with 3 indexes (removed job_id hash -
  // unnecessary!)
  struct by_run_time {};
  struct by_nodes {};

  /** @brief Multi-index container used to query jobs within one block. */
  using JobBlock = boost::multi_index::multi_index_container<
      ...
      boost::multi_index::indexed_by<
          ...
          boost::multi_index::ordered_non_unique<
              ...
              boost::multi_index::member<JobEntry, tdiff_t,
                                         &JobEntry::run_time_estimate>>,

          // Index 2: Ordered by nodes (ascending)
          boost::multi_index::ordered_non_unique<
              boost::multi_index::tag<by_nodes>,
              boost::multi_index::member<JobEntry, num_nodes_t,
                                         &JobEntry::nodes_requested>>>>;

  /** @brief Active entries and query metadata for one queue block. */
  struct BlockInfo {
    ...
    /** @brief Construct an empty block with no active entries. */
    BlockInfo() : active_count(0) {}

    // Query min values directly from multi_index (O(1) - first element in
    // sorted index)
    /** @brief Return the smallest active runtime estimate in this block.
     * @return Minimum estimate, or the maximum tdiff_t value when empty. */
    tdiff_t get_min_run_time() const {
      if (active_count == 0)
        return std::numeric_limits<tdiff_t>::max();
      auto &idx = block.template get<by_run_time>();
      return idx.empty() ? std::numeric_limits<tdiff_t>::max()
                         : idx.begin()->run_time_estimate;
    }

    /** @brief Return the smallest active node request in this block.
     * @return Minimum request, or the maximum num_nodes_t value when empty.
     */
    num_nodes_t get_min_nodes() const {
      if (active_count == 0)
        return std::numeric_limits<num_nodes_t>::max();
      auto &idx = block.template get<by_nodes>();
      return idx.empty() ? std::numeric_limits<num_nodes_t>::max()
                         : idx.begin()->nodes_requested;
    }
  };

  /** @brief Compute floor(log2(value)) for a compile-time block size.
   * @param[in] value Positive integer value.
   * @return Base-two logarithm rounded down as size_t. */
...
BlockWaitQueue<BlockSize>::find_and_remove_backfill_candidate(
  ...
  for (auto &block_info : m_blocks) {
    ...
    for (auto it = seq.begin(); it != seq.end(); ++it) {
      ...
      if (it->submit_time > current_time)
        continue;
      if (it->nodes_requested > available_nodes)
        continue;
