...
Job_Stat_Submit::Context::Context(const Trace &trace)
    : m_it_end(trace.get_reserved().cend()),
      ...
      m_week_end(
          get_time_of_cur_week_start(trace.data().at(0).get_submit_time())),
      m_week_start(m_week_end), m_ratio(static_cast<avail_t>(1.0)),
...
void Job_Stat_Submit::Context::calc_availability() {
  ...
  if (m_dat_continue) {
    if (m_slot_end <= dat_end) {
      ...
      m_ratio -= static_cast<avail_t>(std::difftime(dat_end, m_slot_start) /
                                      hour_in_sec);
      m_dat_continue = false;
    ...
    if (dat_start < m_slot_end) {
      ...
      if (m_slot_end <= dat_end) {
        m_ratio -= static_cast<avail_t>(std::difftime(m_slot_end, dat_start) /
                                        hour_in_sec);
        m_dat_continue = true;
      } else { // It also ends in this slot
        m_ratio -= static_cast<avail_t>(std::difftime(dat_end, dat_start) /
                                        hour_in_sec);
        m_dat_continue = false;
        m_it++; // Check the next DAT
        calc_availability();
...
void Job_Stat_Submit::process(const Trace &trace) {
  ...
  const auto num_weeks = static_cast<size_t>(std::ceil(
      std::ceil(jobs.back().get_end_time() - jobs.front().get_submit_time()) /
      Context::week_in_sec));
...
Job_Stat_Submit::summary_week_t Job_Stat_Submit::get_summary() const {
    ...
    for (size_t wd = 0ul; wd < 7ul; ++wd) {
      for (size_t hr = 0ul; hr < 24ul; ++hr) {
        ...
        if (av == static_cast<avail_t>(0.0)) {
          ...
          sum.m_avg_submit +=
              static_cast<Summary::avg_submit_t>(static_cast<double>(nj) / av);
          sum.m_avg_avail += av;
  ...
  for (size_t s = 0ul; s < summary.size(); ++s) {
    ...
    sum.m_avg_submit_unscaled = static_cast<Summary::avg_submit_t>(
        static_cast<double>(sum.m_tot_submit) / static_cast<double>(num_weeks));
    sum.m_avg_avail =
        static_cast<avail_t>(sum.m_avg_avail / static_cast<double>(num_weeks));
  }
  ...
  for (size_t s = 0ul; s < summary.size(); ++s) {
    ...
    if (num_weeks_effective == static_cast<num_jobs_t>(0u)) {
      ...
      sum.m_std_submit = static_cast<Summary::avg_submit_t>(std::sqrt(
          sum.m_std_submit / static_cast<double>(num_weeks_effective)));
    }
...
std::ostream &
Job_Stat_Submit::print_summary(std::ostream &os,
...
std::ostream &
Job_Stat_Submit::export_submit_data(std::ostream &os,
...
submit_week_t
Job_Stat_Submit::export_submit_data(const bool scale_by_avail) const {
