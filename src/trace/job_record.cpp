...
Job_Record::Job_Record(const Job_Record &o)
    : m_t_begin(o.m_t_begin), m_t_end(o.m_t_end), m_t_submit(o.m_t_submit),
      m_t_limit(o.m_t_limit), m_actual_run_time(o.m_actual_run_time),
      m_num_nodes(o.m_num_nodes), m_q(o.m_q), m_is_simulated(o.m_is_simulated),
#if SHOW_ORG_NO
      m_org_no(o.m_org_no),
#endif
#if MARK_DAT_PERIOD
...
Job_Record::Job_Record(Job_Record &&o) noexcept
    : m_t_begin(std::move(o.m_t_begin)), m_t_end(std::move(o.m_t_end)),
      m_t_submit(std::move(o.m_t_submit)), m_t_limit(std::move(o.m_t_limit)),
      m_actual_run_time(std::move(o.m_actual_run_time)),
      m_num_nodes(std::move(o.m_num_nodes)), m_q(std::move(o.m_q)),
...
Job_Record::Job_Record(const std::vector<std::string> &str_vec,
  ...
  if ((m_num_nodes > BATCH_JOB_NODE_LIMIT) && _Is_Batch(m_q)) {
    throw std::domain_error{
        ...
        " exceeds the limit of num nodes: " + std::to_string(m_num_nodes) +
  ...
  if (is_replay_mode) {
    ...
    // If actual_run_time provided, read it; otherwise will be set by
...
#if BATCH_JOB_NODE_LIMIT
  if ((m_num_nodes > BATCH_JOB_NODE_LIMIT) && _Is_Batch(m_q)) {
    throw std::domain_error{"Batch job exceeds the limit of num nodes: " +
...
std::string Job_Record::to_string() const {
  ...
  std::string str = to_string(m_num_nodes) + ',' + to_string(m_t_begin) + ',' +
