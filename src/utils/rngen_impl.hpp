...
#include <type_traits> // std::is_same, std::is_trivially_copyable

#if defined(DR_EVT_HAS_OPENMP) && !defined(_OPENMP)
...
template <template <typename> typename D, typename V>
inline void RNGen<D, V>::use_seed_seq(const dr_evt::seed_seq_param_t &p) {
  m_sseq_used = true;
  m_sseq_param.clear();
...
#if OMP_DEBUG
  std::vector<dr_evt::my_omp_affinity> omp_aff(m_gen.size());
#endif // OMP_DEBUG
...
#pragma omp parallel num_threads(m_gen.size())
  {
    ...
    if (m_sseq_used) {
      dr_evt::seed_seq_param_t sseq_thread_param;
...
#else  // DR_EVT_THREAD_PRIVATE_RNG
  if (m_sseq_used) {
...
template <template <typename> typename D, typename V>
inline const typename RNGen<D, V>::distribution_t &
RNGen<D, V>::distribution() const {
...
template <template <typename> typename D, typename V>
inline const typename RNGen<D, V>::generator_list_t &
RNGen<D, V>::engine() const {
  return m_gen;
}
#else  // DR_EVT_THREAD_PRIVATE_RNG
template <template <typename> typename D, typename V>
...
#else
  is >> bits(m_seed) >> bits(m_sseq_used) >> bits(m_sseq_param) >>
      bits(m_gen) >> bits(m_distribution);
#endif // DR_EVT_THREAD_PRIVATE_RNG
  return is;
}
