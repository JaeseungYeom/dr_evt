...
bool save_to_streamvec(const MethodT method, const RNGenT &rgen,
                       std::vector<char> &buf,
...
template <typename RNGenT>
bool load_from_streamvec(const MethodT method, RNGenT &rgen,
 ...
 *
 * \tparam<RNGenT>{, the first template parameter, is the type of dr_evt::RNGen
 * object. This class consists of members of various types including boolean,
 * integer, vector, and rng engine, std::distribution structure.}
 *
...
inline bool test_RNGen_state_io(const RNGenParamT &p, const MethodT method,
...
#endif
{
  ...
  SECTION("Save and load the state of RNGen based on integer type uniform "
    ...
    std::stringstream sstr;
    ok = test_RNGen_state_io<rng_uint_t>(rng_uint_t::param_type(100, 999), Bits,
                                         StreamVec, sstr);
    CHECK_RESULT;
  }
  SECTION("Save and load the state of RNGen based on real type uniform "
