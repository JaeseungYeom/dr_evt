...
params_t get_params(int &argc, char **&argv) {
  ...
  std::cout << argv[0] << ' ' << p.num_gen << ' ' << p.seedval << ' '
            << p.idx_sr << ' ' << p.num_sr << std::endl;
  return p;
}
...
int main(int argc, char **argv) {
  if (argc == 1) {
    std::cout << "Usage: > " << argv[0]
              << " num_gen [seed [iter_ckpt [num_save/load]]]" << std::endl;
    return 0;
  }
  ...
  { // compilation test
    ...
    (void)ba;
    // bits_t bb(bits(b)); // does not work because bits() is not defined for
    // std::vector
  }
  ...
  std::cout
      << "-------- test streamvec (separate for input and output) --------"
      << std::endl;
  test_streamvec();

  std::cout << "-------- test streamvec (shared by input and output) --------"
            << std::endl;
  int c = 0x04030201;
  test_iostremavec(c);

  std::cout
      << "-------- test streambuff (separate for input and output) --------"
      << std::endl;
  test_streambuff();

  std::cout << "-------- test streambuff (shared by input and output) --------"
            << std::endl;
  // int c = 0x04030201;
  test_iostremabuff(c);
