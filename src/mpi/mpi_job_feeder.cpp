...
/**
 * @brief Simulation adapter that exposes preloaded-job submission to the MPI
 * feeder.
 * @details The feeder partitions an already loaded Trace among ranks and
 * therefore needs the protected submit_job() path.  Live clients should use
 * append_job(), which creates and submits a record atomically.
 */
...
int main(int argc, char **argv) {
  ...
  int rank; ///< MPI_COMM_WORLD rank of this feeder process.
  int size; ///< Total number of cooperating feeder ranks.
  ...
  std::string trace_file = argv[1]; ///< Input trace path shared by all ranks.
  int total_nodes =
      1000; ///< Simulated cluster capacity; overridden by the CLI.
  ...
  try {
    ...
    std::stable_sort(sim.get_trace().data().begin(),
      ...
      while (next_job_idx < my_job_info.size() &&
        ...
        if (params.m_verbose && jobs_submitted <= 5) {
          std::cout << "[Rank " + std::to_string(rank) + "] Submitted job " +
                           ...
                           " at t=" + std::to_string(global_next_submit) + "\n";
        }
    ...
    MPI_Reduce(&jobs_submitted, &total_submitted, 1, MPI_INT, MPI_SUM, 0,
               MPI_COMM_WORLD);
    ...
    if (rank == 0) {
      std::cout << "\n====================================\n"
                   ...
                   "====================================\n" +
                       std::string("Total jobs submitted: ") +
      ...
      std::cout << "[Rank " + std::to_string(rank) +
                       "] Jobs submitted: " + std::to_string(jobs_submitted) +
                       "\n";
    }
