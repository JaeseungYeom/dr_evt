...
void run_mpi_streaming(const std::string &trace_file, int total_nodes, int rank,
                       int size) {
  ...
  std::cout << "Rank " << rank << ": " << my_jobs.size() << " jobs"
            << std::endl;
  ...
  while (true) {
    ...
    double global_next;
    MPI_Allreduce(&my_next, &global_next, 1, MPI_DOUBLE, MPI_MIN,
                  MPI_COMM_WORLD);
    ...
    if (rank == 0) {
      ...
      for (int r = 1; r < size; r++) {
        ...
        if (!compare_files("/tmp/mpi_stream_rank.0.csv", rank_file.str())) {
          std::cout << "  ✗ Rank 0 and rank " << r << " outputs differ!"
                    << std::endl;
      ...
      if (all_ranks_agree) {
        std::cout << "  ✓ All " << size << " ranks produced identical output"
                  << std::endl;
      }
      ...
      bool matches_batch =
          compare_files("/tmp/mpi_stream_rank.0.csv", "/tmp/mpi_batch.csv");
      ...
      if (success) {
        ...
        if (!all_ranks_agree) {
          std::cout
              << "  ✗ Ranks produced different outputs (non-deterministic!)"
              << std::endl;
        }
        if (!matches_batch) {
          std::cout << "  ✗ MPI streaming differs from batch mode" << std::endl;
